...
async function fetchAllRecords(tableName) {
    ...
    async function runIteration() {
        ...
        do {
            ...
            const response = await fetch(url, {
                ...
                next: { revalidate: AIRTABLE_REVALIDATE },
            });
            ...
            if (!response.ok) {
                ...
                const isIteratorError =
                    response.status === 422 &&
                    errorText.includes("LIST_RECORDS_ITERATOR_NOT_AVAILABLE");

                if (isIteratorError) {
                    throw new Error("AIRTABLE_ITERATOR_EXPIRED");
                }
    ...
    try {
        ...
        if (error instanceof Error && error.message === "AIRTABLE_ITERATOR_EXPIRED") {
            return await runIteration();
        }
        throw error;
    }
...
export const getChansons = cache(async function getChansons() {
...
export const getConcerts = cache(async function getConcerts() {
...
export const getHighlightedConcert = cache(async function getHighlightedConcert() {
